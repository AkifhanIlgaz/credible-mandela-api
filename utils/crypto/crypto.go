...
func VerifySignature(message []byte, signature, from string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("error recovering public key from signature: %w", err)
	}

	recoveredAddress := crypto.PubkeyToAddress(*sigPublicKey)

	if from != recoveredAddress.Hex() {
		return false, fmt.Errorf("signature verification failed: recovered address %s does not match provided address %s", recoveredAddress, from)
	}
